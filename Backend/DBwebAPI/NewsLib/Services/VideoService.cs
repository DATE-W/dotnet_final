...
namespace NewsLib.Services
{
    ...
    {
        public async Task<CustomResponse> GetVideoRandomly(GetVideoRequest json)
        {
            ...
            if (ORACLEConnectTry.getConn() == true)
            {
                try
                {
                    ...
                    return new CustomResponse { ok = "yes", value = videos };
                }
                catch (Exception ex)
                {
        ...
        public async Task<CustomResponse> SearchVideo(SearchVideoRequest json)
        {
